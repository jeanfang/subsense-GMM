...
void BackgroundSubtractorSuBSENSE::initialize(const cv::Mat& oInitImg, const cv::Mat& oROI) {
  ...
  if (oInitImg.type() == CV_8UC3) {
	...
	cv::Mat voIIC[3];
    cv::split(oInitImg, voIIC);
    if (!cv::countNonZero((voIIC[0] != voIIC[1]) | (voIIC[2] != voIIC[1])))
...
Mat BackgroundSubtractorSuBSENSE::process(cv::Mat input, cv :: Mat output, double learningRateOverride, int *labels, int total) {

	//apply(input, output, learningRateOverride);
	Mat subsense = output;
	uint8_t *segmentation_map = output.data;
	int *spc = NULL, *spn = NULL, *fgc = NULL, *fg = NULL;
	spc = (int*)calloc(total, sizeof(int));
	spn = (int*)calloc(total, sizeof(int));
	fgc = (int*)calloc(total, sizeof(int));
	fg = (int*)calloc(total, sizeof(int));
	...
	for (int kk = 0; kk<width*height; kk++)
	{
		if (segmentation_map[kk]>0)
			fg[labels[kk]]++;
		spn[labels[kk]]++;
		if (double(spn[labels[kk]]) / double(spc[labels[kk]]) > 0.8)
			if (double(fg[labels[kk]]) / (double)spn[labels[kk]] > 0.5)
				fgc[labels[kk]] = UCHAR_MAX;
	}
	...
	for (uint8_t *mask = segmentation_map; mask < segmentation_map + (width * height); ++mask,++kk)
		if (*mask > 0 || fgc[labels[kk]] == UCHAR_MAX) *mask = UCHAR_MAX;
	//imshow("Original-Foreground", output);
	if (spc)
	{
		free(spc); spc = NULL;
	}
