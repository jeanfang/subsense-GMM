...
int32_t libvibeModel_Sequential_Segmentation_8u_C1R(
	...
	/*for (int kk = 0; kk<width*height; kk++)
	{
		if (segmentation_map[kk]>0)
			fg[labels[kk]]++;
		spn[labels[kk]]++;
		double nc = double(spn[labels[kk]]) / double(spc[labels[kk]]);//there may be a problem
		double fs = double(fg[labels[kk]]) / (double)spn[labels[kk]];//same above
		if (nc > 0.8)
			if (fs > 0.5) {
				fgc[labels[kk]] = COLOR_FOREGROUND;
				//cout << kk << " " << labels[kk] <<" "<< fgc[labels[kk]]  << endl;
			}
